...
def build_trait_simdata(
    ...
    int64_values = ages_values + species_values  # todos na mesma tabela
    ...
    for i, (col_name, dt, _) in enumerate(TRAIT_COLUMNS):
        ...
        if col_name == 'ages':
            ...
            patch_u32(field_pos + 4, 1)
                          ...
                          'conflicting_traits', 'genders', 'tags'):
            ...
            # Variant: [int32 offset NULL, uint32 typeHash]
            patch_i32(field_pos, RELOFFSET_NULL)
            patch_u32(field_pos + 4, 0x603EAA6C)  # hash do tipo (do trait.xml)
