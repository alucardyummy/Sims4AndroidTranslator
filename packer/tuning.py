...
def fnv32(text: str) -> int:
    """
    Hash FNV-1a 32 bits — implementação correta do Sims 4.
    Usa UTF-16BE (word por word), lowercase, '.' substituído por '_'.
    Baseado no helpers.py do S4Studio Blender Plugin.
    """
    import struct
    from functools import reduce
    encoded = (text.lower().replace('.', '_') if text else '').encode('utf-16be')
    words = [struct.unpack('>H', encoded[i:i+2])[0] for i in range(0, len(encoded), 2)]
    return reduce(lambda v, w: (v * _FNV32_PRIME ^ w) & _MASK32, words, _FNV32_OFFSET)


def fnv64(text: str) -> int:
    """
    Hash FNV-1a 64 bits — implementação correta do Sims 4.
    Usa UTF-16BE (word por word), lowercase, '.' substituído por '_'.
    Baseado no helpers.py do S4Studio Blender Plugin.
    """
    import struct
    ...
    encoded = (text.lower().replace('.', '_') if text else '').encode('utf-16be')
    words = [struct.unpack('>H', encoded[i:i+2])[0] for i in range(0, len(encoded), 2)]
    return reduce(lambda v, w: (v * _FNV64_PRIME ^ w) & _MASK64, words, _FNV64_OFFSET)
...
def build_trait_xml(
    ...
    # Tags de grupo do CAS — controlam em qual aba o traço aparece
    # Valores confirmados do S4Studio.dll:
    # TraitGroup_Emotional=753, TraitGroup_Hobbies=754,
    # TraitGroup_Lifestyle=755, TraitGroup_Social=756
    TAGS_MAP = {
    ...
    tags_list = TAGS_MAP.get(trait_type, [753])
    tags_el = ET.SubElement(root, 'L')
    tags_el.set('n', 'tags')
    for tag_val in tags_list:
        t_el = ET.SubElement(tags_el, 'T')
        t_el.text = str(tag_val)
